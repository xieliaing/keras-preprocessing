...
def text_to_word_sequence_cn(text, filters='!"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n',):
    """
    Transform each text in a list of Chinese texts to a sequences of Chinese words
    text splitting is done using Jieba package.
    """    
...
def texts_to_sequences_cn(texts, num_words, oov_token, document_count, filters, word_index ):
    """Transforms each text in Chinese texts to a sequence of integers.
    Only top `num_words-1` most frequent words will be taken into account.
    Only words known by the tokenizer will be taken into account.
    # Arguments
        texts: A list of Chinese texts (strings).
    # Returns
        A list of sequences.
    """
...
def texts_to_sequences_generator_cn(texts, num_words, oov_token, document_count, filters, word_index ):
    ...
    for text in texts:
        ...
        if isinstance(text, list):
            longtext = ' '.join(text)           
            text = longtext
        seq = text_to_word_sequence_cn(text, filters)
        #print(seq, '\n')
